...
This script recursively scans for Manifest files,  searches for media and extracts them into folder of the users choosing.

It then parses the Manifest.mbdb file which later versions of IOS use to store backup information



Check it out from git, be sure python is in your path, and this script should do the rest, automagically
...
def backupFromDatabase( file ):
	...
	for offset, fileinfo in mbdb.items():
		if offset in mbdx:
			...
			if re.match(ur"^[^A][p][p]", fileinfo['domain']) and fileinfo['filelen'] > 0 and ((fileinfo['mode'] & 0xE000) == 0x8000):# and re.match(ur"[^THM]", fileinfo['filename']):				
...
if __name__ == '__main__':
	...
	for domain in sorted(sizes, key=sizes.get):
		if( sizes[domain] > 1024 * 1024 ):
			print "%-60s Extracted: (%dMB)" % (domain, int(sizes[domain]/1024/1024))
